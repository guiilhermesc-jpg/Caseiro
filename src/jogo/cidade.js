...
export function criaCidade() {
  ...
  // ruas em GRADE (ruas padrão) + praça central
  const ruaMat = mat(0x595f66, 1);
  const faixaH = (z) => { const m = new THREE.Mesh(new THREE.BoxGeometry(180, 0.16, 8), ruaMat); m.position.set(0, 0.08, z); m.receiveShadow = true; scene.add(m); };
  const faixaV = (x) => { const m = new THREE.Mesh(new THREE.BoxGeometry(8, 0.16, 180), ruaMat); m.position.set(x, 0.08, 0); m.receiveShadow = true; scene.add(m); };
  [-48, -16, 16, 48].forEach((c) => { faixaH(c); faixaV(c); });

  const praca = new THREE.Mesh(new THREE.BoxGeometry(30, 0.2, 30), mat(0x9a9082, 1));
  praca.position.y = 0.1; praca.receiveShadow = true; scene.add(praca);

  const obstaculos = [], solidos = [], aguas = [], postes = [], nuvens = [];
  const add = (res) => {
  ...
  // marcos (todos virados PARA a praça)
  ...
  // casas diversas, alinhadas na grade e viradas pro centro
  ...
  lotes.forEach(([x, z]) => add(criaPredio({
    ...
    cor: pick(cores), corTelhado: pick(telhados), rot: Math.atan2(-x, -z),
  })));
  ...
  // pinheiros + arbustos (com flores) nos cantos dos quarteirões
  ...
  return { scene, sun, hemi, skyMat, obstaculos, solidos, aguas, postes, nuvens };
}
