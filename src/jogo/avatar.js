...
//  Cores personalizáveis (tela de seleção): casaco, pele, cabelo.
// =============================================================
...
export function criaAvatar(cores = {}) {
  ...
  const torso = new THREE.Mesh(new THREE.BoxGeometry(1.0, 1.2, 0.55), mat(C.casaco));
  ...
  const cabelo = new THREE.Mesh(new THREE.BoxGeometry(0.86, 0.24, 0.84), mat(C.cabelo));
  ...
  [-0.2, 0.2].forEach((ox) => {
    ...
    const sob = new THREE.Mesh(new THREE.BoxGeometry(0.24, 0.05, 0.05), mat(C.cabelo));
  ...
  // braços + mãos
  const bracoEsq = membro(0.3, 0.95, mat(C.casaco)); bracoEsq.position.set(-0.66, 2.05, 0);
  const bracoDir = membro(0.3, 0.95, mat(C.casaco)); bracoDir.position.set(0.66, 2.05, 0);
  [bracoEsq, bracoDir].forEach((b) => {
    const mao = new THREE.Mesh(new THREE.BoxGeometry(0.32, 0.3, 0.32), mat(C.pele));
    mao.position.y = -0.95; mao.castShadow = true; b.add(mao);
  });

  // pernas + botas
  const pernaEsq = membro(0.34, 0.9, mat(C.calca)); pernaEsq.position.set(-0.26, 0.9, 0);
  const pernaDir = membro(0.34, 0.9, mat(C.calca)); pernaDir.position.set(0.26, 0.9, 0);
  [pernaEsq, pernaDir].forEach((p) => {
    const bota = new THREE.Mesh(new THREE.BoxGeometry(0.37, 0.22, 0.52), mat(C.bota));
    bota.position.set(0, -0.79, 0.08); bota.castShadow = true; p.add(bota);
  });
