...
## 🚨 42ª RODADA (EM ABERTO) — PATCH 2 EM ANDAMENTO (atualizado 11/06, RV5.7/v42)
> **PATCH 1 LANÇADO em 11/06/2026** (cliente v38 + servidor com volume — verificados em produção). Patch 2 acumulando: 38ª (portões/casas/chão) + 39ª (entradas/HP/sons) + 40ª (biomas) + 41ª (eventos de mundo). **Deploy do v39-v42 PENDENTE** (produção está no v38).
> 🧪 Testar quando publicar: portões · casas novas/entradas · barra de HP · biomas (juncos/cogumelos/trigo) · ESQUELETOS ERGUENDO no cemitério à noite (e sumindo de dia) · MASCATE acampado num dos 3 pontos da estrada (muda a cada sessão).
> 🔭 Fila do Patch 2: GLBs CC0 do maestro nos slots · feedback dos jogadores · mais masmorras · launcher (Electron/Tauri).
